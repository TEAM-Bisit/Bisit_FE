...
class TossPayActivity : AppCompatActivity() {
    ...
    private fun handlePaymentFailure(orderId: String, failReason: String) {
        val request = kr.bisit.app.data.model.payment.PaymentFailRequest(
            orderId = orderId,
            failReason = failReason
        )
        
        RetrofitClient.getPaymentApi(this).failPayment(request)
            .enqueue(object : Callback<kr.bisit.app.data.model.payment.PaymentFailResponse> {
                override fun onResponse(
                    call: Call<kr.bisit.app.data.model.payment.PaymentFailResponse>,
                    response: Response<kr.bisit.app.data.model.payment.PaymentFailResponse>
                ) {
                    if (response.isSuccessful && response.body()?.success == true) {
                ...
                override fun onFailure(call: Call<kr.bisit.app.data.model.payment.PaymentFailResponse>, t: Throwable) {
